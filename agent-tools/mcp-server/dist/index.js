...
const solanaAddressSchema = z
    .string()
    .describe("Base58-encoded Solana wallet address (32-byte public key, 32-44 characters). " +
...
const channelIdSchema = z
    .string()
    .describe("x402 payment channel identifier (1-128 chars; alphanumeric plus '.', '-', '_'). " +
...
const solanaIdentifierSchema = z
    .string()
    .describe("Solana identifier to report on: either a Base58 wallet address (32-byte public key) or a " +
...
const paymentProofSchema = z
    .string()
    .describe("x402 payment proof to verify: a Base58 Solana transaction signature (64 bytes) or wallet " +
...
const optionalChannelIdSchema = z
    .string()
    .describe("Optional x402 channel identifier (alphanumeric plus '.', '-', '_'). When supplied and " +
 ...
 */
const screeningLedger = new Map();
export function recordScreening(record) {
    screeningLedger.set(record.address, record);
}
export function clearScreeningLedger() {
    screeningLedger.clear();
}
export function getScreeningRecord(address) {
    return screeningLedger.get(address);
}
function toScreeningRecord(result) {
    const lastEvaluated = typeof result.lastEvaluated === "string" && result.lastEvaluated.length > 0
...
function toEvaluationTimestamp(result, fallback) {
...
export function formatComplianceReport(identifier) {
    ...
    if (!record) {
        return JSON.stringify({
            ...
            message: "No active screening record found for this identifier. Run screen_solana_address to execute an authoritative live screening against OFAC and sanctions databases.",
        }, null, 2);
    }
    return JSON.stringify({
        identifier: trimmed,
...
export function verifyPaymentProof(paymentProof, channelId) {
...
export function createServer(client) {
    ...
    server.registerTool(VERIFY_PAYMENT_TOOL_NAME, {
        ...
        try {
            ...
            return {
                content: [{ type: "text", text: formatPaymentVerification(verification) }],
            };
        }
        catch (err) {
            const message = err instanceof Error ? err.message : String(err);
            return {
                isError: true,
                content: [{ type: "text", text: `verify_x402_payment failed: ${message}` }],
            };
        }
    });
    return server;
